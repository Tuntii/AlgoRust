...
// Uses configurable lookback/lookahead. A pivot with `pivot_len = N`
// is only confirmed after N bars of right-side confirmation.

pub fn is_pivot_high(highs: &[Decimal], idx: usize, pivot_len: usize) -> bool {
    if idx < pivot_len || idx + pivot_len >= highs.len() {
        return false;
    }
    let current = highs[idx];
    let left = highs[idx - pivot_len..idx].iter().all(|&h| h < current);
    let right = highs[idx + 1..=idx + pivot_len].iter().all(|&h| h < current);

    left && right
}

pub fn is_pivot_low(lows: &[Decimal], idx: usize, pivot_len: usize) -> bool {
    if idx < pivot_len || idx + pivot_len >= lows.len() {
        return false;
    }
    let current = lows[idx];
    let left = lows[idx - pivot_len..idx].iter().all(|&l| l > current);
    let right = lows[idx + 1..=idx + pivot_len].iter().all(|&l| l > current);
