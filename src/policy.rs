...
impl CooldownManager {
    pub fn new() -> Self {
        Self {
            ...
            min_cooldown: 1, // OPTIMIZED: Reduced from 2
    ...
    /// OPTIMIZED: Reduced cooldowns for faster signal generation
    pub fn get_cooldown_for_tf(&self, timeframe: &str) -> usize {
        let base = if self.is_backtest_mode {
            1
        } else {
            self.min_cooldown
        };
        ...
        match timeframe {
            "5m" | "15m" => base, // Was base + 1 (2-3 candles)
            "30m" | "1h" => base, // Same (1 candle)
            "4h" | "1d" => 1,     // Same (1 candle)
            _ => base,
        }
    }
    ...
    pub fn record_context_close(&mut self, context_id: &ContextId, candle_idx: usize) {
        self.context_cooldowns
            .insert(context_id.to_string(), candle_idx);
    }
    ...
    pub fn remaining_context_cooldown(
        &self,
        context_id: &ContextId,
        timeframe: &str,
        current_candle_idx: usize,
...
impl LiquidityBosAnalyzer {
    ...
    pub fn evaluate(
        ...
        let has_liquidity = if is_bullish {
            has_equal_lows
        } else {
            has_equal_highs
        };
        ...
        if has_liquidity && has_displacement {
            return (
                15,
                true,
                Some("✅ Strong BOS: Liquidity sweep + Displacement (+15)".to_string()),
            );
        }
        ...
        if has_liquidity {
            return (
                10,
                true,
                Some("✅ BOS with liquidity sweep (+10)".to_string()),
            );
        }
        ...
        if has_displacement {
            return (
                10,
                true,
                Some("✅ BOS with strong displacement (+10)".to_string()),
            );
        }
        ...
        (
            -10,
            false,
            Some("⚠️ Weak BOS: No displacement, no liquidity sweep (-10)".to_string()),
        )
    }
}
...
impl DisplacementValidator {
    ...
    pub fn evaluate(&self, candle: &Candle, is_bullish_signal: bool) -> (i32, Option<String>) {
        ...
        if range.is_zero() {
            return (
                -10,
                Some("⚠️ Zero range candle after pivot (-10)".to_string()),
            );
        }
...
impl EthMicroBoost {
    ...
    pub fn evaluate(
        ...
        if has_liquidity_sweep && ema_confluence {
            return (
                5,
                Some("✅ ETH Micro-Boost: Liquidity sweep + EMA confluence (+5)".to_string()),
            );
        }
...
impl PolicyEngine {
    ...
    pub fn evaluate_with_penalties(
        ...
        let (vol_penalty, vol_block, vol_reason) =
            self.volatility_filter
                .evaluate(timeframe, atr_ratio, median_atr_ratio);
        if vol_block {
        ...
        let (slope_penalty, slope_block, slope_reason) =
            self.slope_filter.evaluate(ema_slope, is_low_atr);
        if slope_block {
        ...
        if !passes_threshold {
            reasons.push(format!(
                ...
                final_score,
                timeframe,
                ScoreThreshold::min_score_for_tf(timeframe)
            ));
        }
