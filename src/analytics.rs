...
impl AdvancedMetrics {
    pub fn calculate(trades: &[TradeRecord]) -> Self {
        ...
        let avg_trade_duration_candles = trades
            .iter()
    ...
    fn calc_overlap_risk_metrics(
        trades: &[TradeRecord],
    ) -> (HashMap<u32, f64>, f64, f64, f64, f64) {
        ...
        let overlap_trades: Vec<&TradeRecord> =
            trades.iter().filter(|t| t.was_concurrent).collect();
...
impl RegimeReport {
    pub fn generate(trades: &[TradeRecord]) -> Self {
        ...
        for trade in trades {
            ...
            conf_groups
                .entry(trade.confidence_tier.clone())
                .or_default()
                .push(trade);
            ...
            if let Some(ref regime) = trade.regime {
                atr_groups
                    .entry(regime.atr_regime.clone())
                    .or_default()
                    .push(trade);
                slope_groups
                    .entry(regime.slope_regime.clone())
                    .or_default()
                    .push(trade);
                session_groups
                    .entry(regime.session.clone())
                    .or_default()
                    .push(trade);
            }
        }
        ...
        for (key, group) in slope_groups {
            report
                .by_slope_regime
                .insert(key, Self::calc_bucket(&group));
        }
    ...
    fn calc_bucket(trades: &[&TradeRecord]) -> RegimeBucket {
        ...
        let expectancy =
            ((wins as f64 / count as f64) * avg_win) - ((losses as f64 / count as f64) * avg_loss);
